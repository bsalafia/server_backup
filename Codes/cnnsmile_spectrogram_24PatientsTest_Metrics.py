...
################################################################
  ...
  model.compile(optimizer=Adam(learning_rate=0.001),loss='binary_crossentropy',metrics=[f1_score_met])
  return model
...
#
    ...
    y1=np.ravel(y)
    print("label",y.shape)
    cw = class_weight.compute_class_weight('balanced', classes=np.unique(y1),y=np.ravel(y1))
    print(cw)
...
ModelTrain(dirname,dirname2,SaveResults,SaveHisResults,'CNNSMILE10times_Decay.5_LR.001_24Testfiles_testF1',seq_len=1,cnn=0,smile=1,diff=0,twodcnn=0,spectrogram=0,cnnsmilespec=0,gru=0,epoch=100,epochLoad=0,l2_size=k,drop_size=i,batchSize=j)
