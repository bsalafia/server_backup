...
def ModelTrain(dirname,modeldir,SaveResults,SaveResultsTruePred,modelname,seq_len,cnn,smile,diff,twodcnn,indx,gru,Noepoch,l2_size,drop_size,batchSize):
  ...
  for th in [0.7]:
    ...
    for trainindx, testindx in kfold.split(range(24)):
      ...
      ModelName1=modelname+'_fold'+str(fold_no)+'_epoch_'+str(Noepoch)+'_l2Size_'+str(l2_size)+'_DropSize_'+str(drop_size)+'_batchsize_'+str(batchSize)+'.h5'
  ...
  for j in range(3):
      ...
      plt.suptitle(modelname+'_EDFNo_'+str(j+1)+'_epoch_'+str(Noepoch)+'_'+indx+'_l2Size_'+str(l2_size)+'_DropSize_'+str(drop_size)+'_batchsize_'+str(batchSize))

      plt.savefig(SaveResultsTruePred+'/'+modelname+'_EDFNo_'+str(j+1)+'_epoch_'+str(Noepoch)+'_'+indx+'_l2Size_'+str(l2_size)+'_DropSize_'+str(drop_size)+'_batchsize_'+str(batchSize)+'.pdf', format='pdf', bbox_inches = 'tight')
      plt.clf()
...
for i in [0.1,0.25,0.5]:
  ...
  for j in [256,512]:
    ...
    for k in [0.001,0.01]:
      ...
      modeldir='/home/baharsalafian/CNNSMILEGRU100Epoch_L2DropExperiment_NewVersion'

      # SaveHisResults='/home/baharsalafian/HistoryResExperiment_NewVersion'

      SaveResults='/home/baharsalafian/Results_L2DropExperiment_NewVersion'
      SaveResultsTruePred='/home/baharsalafian/TruePredPlots_Experiment_NewVersion'


      ModelTrain(dirname,modeldir,SaveResults,SaveResultsTruePred,'CNNSMILEDiff3times',seq_len=1,cnn=0,smile=0,diff=1,twodcnn=0,indx='test',gru=0,Noepoch=200,l2_size=k,drop_size=i,batchSize=j)
