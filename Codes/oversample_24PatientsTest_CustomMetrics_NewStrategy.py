...
def CNN_GRU(filtNo1,filtNo2,filtsize,input_shape,activation,PoolSize,denseSize,l2_size,drop_size,stddev):
...
def Make_layers(filtNo1,filtNo2,filtsize,input_shape,activation,PoolSize,denseSize,l2_size,drop_size,stddev):
  input1 = Input(input_shape)
...
def CNN_Spectrogram_SMILEMat(initLR,filtNo1,filtNo2,filtsize,input_shape1,activation,PoolSize,denseSize,l2_size,drop_size,stddev):

  # metrics=METRICS
  input1=Input(input_shape1)
  # input2=Input(input_shape2)

  # spec_out=Make_layers(filtNo1,filtNo2,filtsize,input_shape1,activation,PoolSize,denseSize,l2_size,drop_size,stddev)
  # input1 = Input(input_shape)
...
def CNN_SMILE_NewArch(filtNo1,filtNo2,filtsize,input_shape,activation,PoolSize,denseSize,l2_size,drop_size,stddev):
...
def Make_X(x,mi,spectrogram,cnn,smile,spect,cnnsmilespec,smile_new_arch,smile_spec_new_arch,twodcnn,spect_smilemat,eeg_smilemat):
  ...
  if spect_smilemat==1:
    ...
    X_train=np.concatenate((spectrogram,mi),axis=1)
...
def make_batches(x_one,x_zero,y_one,y_zero,mi_one,mi_zero,spectrogram_one,spectrogram_zero,mini_batch,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn):
  ...
  X_main=Make_X( x_batch,mi_batch,spectrogram_batch,cnn,smile,spect,cnnsmilespec,smile_new_arch,smile_spec_new_arch,twodcnn,spect_smilemat,eeg_smilemat)
...
def validation_batches(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,twodcnn):
  ...
  X_main=Make_X(x_one_batch,mi_one_batch,spectrogram_one_batch,cnn,smile,spect,cnnsmilespec,smile_new_arch,smile_spec_new_arch,twodcnn,spect_smilemat,eeg_smilemat)
...
def Select_Model(initLR,cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,smile_spec_new_arch,twodcnn,diff,l2size,dropsize,stddev,spect_smilemat,eeg_smilemat):
  ...
  if smile_new_arch==1:
    tf.keras.backend.clear_session()
    model=CNN_SMILE_NewArch(filtNo1=120,filtNo2=160,filtsize=5,input_shape=(1024,18),activation='relu',PoolSize=3,denseSize=8,l2_size=l2size,drop_size=dropsize,stddev=stddev)
  ...
  if spect_smilemat==1:
    ...
    tf.keras.backend.clear_session()
    model=CNN_Spectrogram_SMILEMat(initLR,filtNo1=120,filtNo2=160,filtsize=5,input_shape1=(1042,18),activation='relu',PoolSize=3,denseSize=8,l2_size=l2size,drop_size=dropsize,stddev=stddev)
...
SaveHisResults='/home/baharsalafian/NewStrategy_results_Allcase'
ModelResults='/home/baharsalafian/NewStrategy_model_Allcase'

modelname='CNN_24Testfiles_LR_'
initLR=0.01
...
for i in range(2):
  ...
  tf.keras.backend.clear_session()
  model=Select_Model(initLR,cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,smile_spec_new_arch,twodcnn,diff,l2size,dropsize,stddev,spect_smilemat,eeg_smilemat)
...
# ... Define a model
  wandb.init(project=modelname+str(initLR)+"_l2size_"+str(l2size)+"_dropsize_"+str(dropsize)+"_Noise_"+str(stddev), entity="bsalafian")
  ...
  model.save(ModelResults+'/'+modelname+str(initLR)+'_fold'+str(i+1)+'_epoch_'+str(num_epochs)+'_batchsize_'+str(batch_size)+'_l2size_'+str(l2size)+'_dropsize_'+str(dropsize)+'Noise'+str(stddev)+'.h5')
