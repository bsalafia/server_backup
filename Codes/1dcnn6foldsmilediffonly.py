...
def ReadMatFiles(dirname1,dirname2,indx):
...
dirname1='/home/baharsalafian/6FoldCrossSMILE'
...
#
  X_train,Y_train,X_diff_train = ReadMatFiles(dirname1,dirname2,trainindx)

  X_test,Y_test,X_diff_test = ReadMatFiles(dirname1,dirname2,testindx)
  model.fit([X_train,X_diff_train],Y_train,validation_split=0.2,batch_size=batchsize , epochs=epoch,verbose = 2)

  X_train = None
  Y_train = None
  gc.collect()

  loss, acc = model.evaluate([X_test,X_diff_test],Y_test, verbose=2)
  score.append(acc)
  loss1.append(loss)
  model.save('/home/baharsalafian/CNNSMILE6FoldRes/CNN6foldsmilediffonlyV2'+ str(fold_no)+'.h5')
  fold_no=fold_no+1
...
np.save(os.path.join(SaveResults,'accuracyCNN6foldsmilediffonlyV2'),score)
np.save(os.path.join(SaveResults,'lossCNN6foldsmilediffonlyV2'),loss1)
