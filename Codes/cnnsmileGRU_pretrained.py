...
def define_model_CNNSMILEGRU(dirPretrained,modelnamePretrained,fold_no,epoch):
...
##################################################################
  ...
  modelpretrained=tf.keras.models.load_model(os.path.join(dirPretrained,ModelName1))

  weights=modelpretrained.get_weights()
  cnn.set_weights(weights[:42])

  #
  # weights=modelpretrained.get_weights()
  # cnn=cnn.set_weights(weights[:42])

  # config = cnn.get_config() # Returns pretty much every information about your model
  # inputcnn=config["layers"][-1]["config"]["batch_input_shape"]
  # print(inputcnn)
...
def define_model_CNNSMILEDiffGRU(dirPretrained,modelnamePretrained,fold_no,epoch):
...
def define_model_CNNGRU(dirPretrained,modelnamePretrained,fold_no,epoch):
...
def ModelTrain(dirname,SaveHisResults,SaveResults,dirPretrained,modelnamePretrained,modelname,seq_len,cnn,smile,diff,epoch):
  ...
  for trainindx, testindx in kfold.split(range(24)):
    ...
    if cnn==1:
      ...
      model=define_model_CNNGRU(dirPretrained,modelnamePretrained,fold_no,epoch)
      X_train=x
      X_test=xtest

    if smile==1:
      tf.keras.backend.clear_session()
      model=define_model_CNNSMILEGRU(dirPretrained,modelnamePretrained,fold_no,epoch)
    ...
    if diff==1:
      ...
      tf.keras.backend.clear_session()
      model=define_model_CNNSMILEDiffGRU(dirPretrained,modelnamePretrained,fold_no,epoch)
  ...
  # print(y)
    ...
    history=model.fit(X_train, y, validation_data=(X_test,ytest), batch_size=batchsize, epochs=epoch, sample_weight=sample_weight, verbose = 2)
...
#############################################
  ...
  plt.title(modelname+'_epoch_'+str(epoch))
  ...
  plt.ylabel('%')
  plt.xlabel('epoch')
  plt.savefig(SaveHisResults+'/'+'history_'+modelname+'_accuracy_'+'_epoch_'+str(epoch)+'.pdf', format='pdf', bbox_inches = 'tight')
  plt.clf()
  ...
  plt.ylabel('%')
  plt.xlabel('epoch')
  plt.savefig(SaveHisResults+'/'+'history_'+modelname+'_loss_'+'_epoch_'+str(epoch)+'.pdf', format='pdf', bbox_inches = 'tight')
  plt.clf()
...
dirPretrained='/home/baharsalafian/CNNSMILEGRU100Epoch_new'

SaveResults='/home/baharsalafian/CNNSMILEGRU100Epoch_Pretrained'

SaveHisResults='/home/baharsalafian/HistoryRes_Pretrained'
# seq_len=7
modelnamePretrained1='CNNSMILEDiff10times'
ModelTrain(dirname,SaveHisResults,SaveResults,dirPretrained,modelnamePretrained=modelnamePretrained1,modelname=modelnamePretrained1+'_Pretrained_GRU'+'10times',seq_len=3,cnn=0,smile=0,diff=1,epoch=100)
