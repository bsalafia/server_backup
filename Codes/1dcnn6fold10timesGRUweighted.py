...
##################################################################
  ...
  encoded_sequence = Bidirectional(GRU(50, return_sequences=True))(encoded_frames)
  output=TimeDistributed(Dense(1,activation='softmax'))(encoded_sequence)
...
dirname='/home/baharsalafian/GRUData3times'
...
#
  ...
  sample_weight = np.ones(ytrain.shape)

  # ytrain1=ytrain
  # ytrain1[:,:,0]=ytrain
  # sample_weight[ytrain == 1] = 6.0
  a= np.unique(ytrain )
  b=np.ravel(ytrain )
  print(ytrain.shape)
  from sklearn.utils import class_weight
  y_ints = [y.argmax() for y in ytrain]
  print(np.array(y_ints).shape)
  cw = class_weight.compute_class_weight('balanced', np.unique(y_ints),np.ravel(y_ints))
  class_weights = {0:cw[0],
                  1:cw[1],
                  2:cw[2]}
  # print(cw)
#   cw2=np.zeros((3,2))
...
#   cw2[2,:]=cw
#   print(cw2)
  model.fit(X_train, ytrain, validation_split=0.2, batch_size=batchsize , epochs=epoch,class_weight=class_weights, verbose=2)

  X_train = None
  Y_train = None
  gc.collect()

  loss, acc = model.evaluate(X_test,ytest, verbose=2)
  score.append(acc)
  loss1.append(loss)
  model.save('/home/baharsalafian/CNNSMILEGRURes3times/CNNGRUweighted3times'+ str(fold_no)+'.h5')
  fold_no=fold_no+1
...
np.save(os.path.join(SaveResults,'accuracyCNNGRUweighted3times'),score)
np.save(os.path.join(SaveResults,'lossCNNGRUweighted3times'),loss1)
