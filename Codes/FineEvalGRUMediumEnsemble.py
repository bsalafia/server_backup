...
def ModelTrain(dirname,SaveResults,modeldir,indx,modelname,batchsize,epoch):
  ...
  for th in [0.3,0.4,0.5,0.6,0.7]:
    ...
    for trainindx, testindx in kfold.split(range(24)):
      ...
      fold_no=fold_no+1
      modelnamecnn='CNNGRU10times_FineTune_'
      modelnamesmile='CNNSMILEGRU10times_FineTune_'
      modelnamediff='CNNSMILEDiffGRU10times_FineTune_'
      ytest_cnn,ypred_cnn=ReadMatFiles(dirname,SaveResults,modeldir,indx,testindx, fold_no,modelnamecnn,batchsize,epoch ,cnn=1,smile=0,diff=0,seq_len=3,twodcnn=0)
      ytest_smile,ypred_smile=ReadMatFiles(dirname,SaveResults,modeldir,indx,testindx, fold_no,modelnamesmile,batchsize,epoch ,cnn=0,smile=1,diff=0,seq_len=3,twodcnn=0)
      ytest_diff,ypred_diff=ReadMatFiles(dirname,SaveResults,modeldir,indx,testindx, fold_no,modelnamediff,batchsize,epoch ,cnn=0,smile=0,diff=1,seq_len=3,twodcnn=0)

      for i in range(len(ytest_cnn)):

        yp_cnn= ypred_cnn[i][:,2]   ###### Get the last element of the sequence of length 3
        yt_cnn= ytest_cnn[i][:,2]

        yp_smile= ypred_smile[i][:,2]   ###### Get the last element of the sequence of length 3
        yt_smile= ytest_smile[i][:,2]

        yp_diff= ypred_diff[i][:,2]   ###### Get the last element of the sequence of length 3
        yt_diff= ytest_diff[i][:,2]
...
# seq_len=7
ModelTrain(dirname,SaveResults,modeldir,'test_finetune_medium_ensemble','EnsembleModelGRU',batchsize=256,epoch=3)
