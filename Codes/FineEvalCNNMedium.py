...
def dataloader(x,mi,y,seq_len, cnn,smile,diff):
...
def ReadMatFiles(dirname,SaveResults,indx,fold_no,modelname,batchsize,epoch ,cnn,smile,diff,twodcnn,seq_len):
  ...
  for k in range(len(ind)):
    ...
    ypred,model=FineTuning(SaveResults,modelname,fold_no,x1,y1,mi1,mi_diff1,batchsize,epoch,Xtest,MItest,MItest_diff,cnn,smile,diff,twodcnn)

    Ypred_all.append(ypred)
    Ytest_all.append(Ytest)


  return Ytest_all,Ypred_all,model



def ModelTrain(dirname,SaveResults,SaveLabels,modeldir,indx,modelname,batchsize,epoch,cnn,smile,diff,twodcnn,seq_len):
  ...
  for th in [0.3,0.4,0.5,0.6,0.7]:
    ...
    for trainindx, testindx in kfold.split(range(24)):
      ...
      ytest,ypred,model=ReadMatFiles(dirname,modeldir,trainindx, fold_no,modelname,batchsize,epoch ,cnn,smile,diff,twodcnn,seq_len)
      print(len(ypred))
      np.save(os.path.join(SaveLabels, 'ytest'+modelname+'_'+indx+'_'+str(fold_no)), ytest)
      np.save(os.path.join(SaveLabels,  'ypred'+modelname+'_'+indx+'_'+str(fold_no)),  ypred)

      # model.save(SaveLabels+'/'+modelname+'_FineTune_'+str(fold_no)+'.h5')
      for i in range(len(ytest)):

        yp= ypred[i]   ###### Get the last element of the sequence of length 3
        yt= ytest[i]
...
SaveLabels='/home/baharsalafian/SavePredLabels'

modeldir='/home/baharsalafian/CNNSMILEGRUCorrectedNoweight'
# seq_len=7
ModelTrain(dirname,SaveResults,SaveLabels,modeldir,'train_finetune_medium_Noweight','2DCNN10timesNoweight',batchsize=256,epoch=3,cnn=0,smile=0,diff=0,twodcnn=1,seq_len=1)
