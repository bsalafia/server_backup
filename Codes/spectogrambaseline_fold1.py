...
def Label(dirname,Name,WindowSize,DenoisedSig):
...
def DFT_Calculate(dirname,savedir,WindowSize,train_indx,lowcut,highcut,fs):
  ...
  for z in train[train_indx]:
    indices,EDFFiles=ReadMatFiles(dirname,z)

    for file_idx in range(len(indices)):

      Name=EDFFiles[indices[file_idx]]

      x=butterworth_denoise(dirname,Name,WindowSize,fs,lowcut,highcut)

      X,Y=Label(dirname,Name,WindowSize,x)

      X_DFT=Get_DFT(X)

      Name=EDFFiles[indices[file_idx]].split('.')

      savemat(os.path.join(savedir, Name[0]+'_Spectogram.mat'), {"spectogram":X_DFT,"Label": Y})
...
savedir='/home/baharsalafian/SpectogramResults'

DFT_Calculate(dirname,savedir,WindowSize=4,train_indx=5,lowcut=0.5,highcut=127,fs=256)
