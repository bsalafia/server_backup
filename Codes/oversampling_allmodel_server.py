...
modelname='CNNSMILE_24Testfiles_Nodecay_TfModel'
initLR=0.1
cnn=0
smile=1
spectrogram=0
...
# EDFFiles=PatientsEDFFile(dirname)
  ...
  # num_epochs = 2
  tf.keras.backend.clear_session()
  Select_Model_on=tf.function(Select_Model)
  model=Select_Model_on(cnn,smile,spectrogram,cnnsmilespec,twodcnn,diff,drop_size)
...
# model=define_SMILE(tf.constant(drop_size))
  ...
  for epoch in range(num_epochs):
    ...
    for step,(x_batch_test, y_batch_test,mi_test,spec_test) in enumerate(test_data):
    ...
    print("new LR", lr_new)
    optimizer.learning_rate=lr_new
    print(f"Test: accuracy over epoch= {test_acc:.4f}, ROC= {test_roc:.4f}, PR= {test_pr:.4f} , Precision={test_precision:.4f}, Recall={test_recall:.4f}, f1score={test_f1score:.4f} , loss_train={test_loss:.4f}")
