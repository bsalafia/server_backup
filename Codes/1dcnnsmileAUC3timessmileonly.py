...
SaveResults='/home/baharsalafian/SMILEvaluateRes'
...
FoldNum=6
model='CNN6foldsmilediffonly3times'
savename='fscore_smile'
threshold=0.03
kfold = KFold(n_splits=FoldNum, shuffle=False)
th=0.8
for trainindx, testindx in kfold.split(range(24)):

  X_test,Y_test,mi_test,midiff = ReadMatFilesDiff(dirname,testindx)
  ...
  ypred1=model1.predict([mi_test,midiff])
...
np.save(os.path.join(SaveResults, 'fpr_cnnsmileonlydiff'),  fpr)
np.save(os.path.join(SaveResults, 'tpr_cnnsmileonlydiff'),  tpr)
np.save(os.path.join(SaveResults, 'PR_cnnsmileonlydiff'), PR)
np.save(os.path.join(SaveResults, 'ROC_cnnsmileonlydiff'),  ROC)
