...
################################################################
  ...
  model.compile(optimizer=Adam(learning_rate=0.0001),loss='binary_crossentropy',metrics=["accuracy", tf.keras.metrics.AUC(curve="ROC",name='auc_roc'),tf.keras.metrics.AUC(curve="PR",name='auc_pr'),f1_score_met])
  return model
...
def define_CNN(l2_size,drop_size):
  ...
  model.compile(optimizer=Adam(learning_rate=0.0001),loss='binary_crossentropy', metrics=["accuracy",f1_score,precision,recall2])
  return model
...
def CNN_Spectrogram(filtNo1,filtNo2,filtsize,input_shape,activation,PoolSize,denseSize):
  ...
  model = Model(inputs=input1, outputs=output)
  model.compile(optimizer=Adam(learning_rate=0.0001),loss='binary_crossentropy', metrics=["accuracy", tf.keras.metrics.AUC(curve="ROC",name='auc_roc'),tf.keras.metrics.AUC(curve="PR",name='auc_pr'),f1])
  return model
...
#
    ...
    sample_weights[y == 0] = cw[0]
    sample_weights[y == 1] = cw[1]
...
########################################
    ...
    wandb.init(project=modelname+"class_weight", entity="bsalafian")

    if gru==1:
      history=model.fit(X_train, y, validation_data=(X_test,ytest), batch_size=batchsize, epochs=epoch, sample_weight=sample_weight,verbose=2,callbacks=[WandbCallback(),LearningRateScheduler(lr_exp_decay)])
    else:

      history=model.fit(X_train, y, validation_data=(X_test,ytest), batch_size=batchsize, epochs=epoch, class_weight=class_weights,verbose=2,callbacks=[WandbCallback(),LearningRateScheduler(lr_exp_decay)])
...
SaveResults='/home/baharsalafian/LastExperiment_results_Allcase'
SaveHisResults='/home/baharsalafian/LastExperiment_model_Allcase'

ModelTrain(dirname,dirname2,SaveResults,SaveHisResults,'CNN_Class_weight_24Patients_fit',seq_len=1,cnn=1,smile=0,diff=0,twodcnn=0,spectrogram=0,cnnsmilespec=0,gru=0,epoch=100,epochLoad=0,l2_size=k,drop_size=i,batchSize=j)
