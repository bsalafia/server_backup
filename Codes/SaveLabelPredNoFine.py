...
def ModelTrain(dirname,modeldir,SaveResults,modelname,seq_len,cnn,smile,diff,twodcnn,indx):
  ...
  for trainindx, testindx in kfold.split(range(24)):
    ...
    for i in range(len(x)):
      ...
      if smile==1:
        X_train=[x[i],mi[i]]
      if diff==1:
        X_train=[x[i],mi[i],mi_diff[i]]
      if twodcnn==1:
        x2d=x[i].reshape(x[i].shape[0],x[i].shape[2],x[i].shape[1],1)
        X_train=x2d
    ...
    np.savez(os.path.join(SaveResults, 'LabelPre_'+modelname+'_'+indx+'_'+str(fold_no)), Ytrue=y, Ypred=ypred)
...
modeldir='/home/baharsalafian/CNNSMILEGRUCorrected'
SaveResults='/home/baharsalafian/LabelPred'

ModelTrain(dirname,modeldir,SaveResults,'CNNGRU10times',seq_len=3,cnn=1,smile=0,diff=0,twodcnn=0,indx='train')
