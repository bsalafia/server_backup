...
def DFT_Calculate(dirname,savedir,WindowSize1,WindowSize2,train_indx,lowcut,highcut,fs):
  ...
  for z in train[train_indx]:
    ...
    indices,EDFFiles2=ReadMatFiles(savedir,z)

    for file_idx in range(len(indices)):

      Name=EDFFiles[indices[file_idx]]

      Name2=EDFFiles2[indices[file_idx]]

      matfile=loadmat(os.path.join(dirname,Name))
      X=matfile['X_4sec']
      Name=EDFFiles[indices[file_idx]].split('.')

      matfile2=loadmat(os.path.join(savedir,Name[0]+'_Spectogram.mat'))
      X2=matfile2['spectogram']


      print(X.shape[0],X2.shape[0])
      ...
      # Name=EDFFiles[indices[file_idx]].split('.')
      #
      # savemat(os.path.join(savedir, Name[0]+'_Spectogram.mat'), {"spectogram":X_DFT})
