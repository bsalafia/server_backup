...
Original file is located at
    https://colab.research.google.com/drive/1txA9IS_XhcJZ7JK1Qi2lNslEQXc9iPdP
"""
    ...
    for j in range(X_train.shape[2]-1):
      for k in range(j+1, X_train.shape[2]):
        ...
        for i in tqdm(range(X_train.shape[0])):
          if Y_train[i] == l:
            continue
          X =(X_train[i, :, j]-mean1[j])/stdv[j]
          Y = (X_train[i, :, k]-mean1[k])/stdv[k]
          estimated_mi[i] =bi_ksg_mi(X,Y,5)
          estimated_MI_per4Sec[l,i,j,k]=estimated_mi[i]
        estimated_MI[l,j,k]=np.mean(estimated_mi)
        estimated_MI_var[l,j,k]=np.var(estimated_mi)

        file_name = 'Pycit_MeanMI_Est_std_1NNperChan_fileidx_{}.mat'.format(file_idex)
        savemat(os.path.join(dir_mi_results, file_name), {"estimated_MI": estimated_MI})

        file_name2 = 'Pycit_VarMI_Est_std_1NNperChan_fileidx_{}.mat'.format(file_idex)
