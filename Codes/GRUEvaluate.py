...
def ModelTrain(dirname,SaveResults,modelname,seq_len,cnn,smile,diff,indx):
  fpr=[]
  tpr=[]
  PR=[]
  ROC=[]
  ...
  for th in [0.3,0.4,0.5,0.6,0.7]:
    ...
    for trainindx, testindx in kfold.split(range(24)):
      ...
      x, y, mi,mi_diff=ReadMatFiles(dirname,testindx,seq_len,diff)
      if cnn==1:
      ...
      ModelName1=modelname+str(fold_no)+'.h5'

      model1=tf.keras.models.load_model(os.path.join(SaveResults,ModelName1))
      ...
      ypred= ypred[:, 2]   ###### Get the last element of the sequence of length 3
      ypred1= ypred1[:, 2]

      if th==0.7:

        fpr1, tpr1, _ = roc_curve(y, ypred1)
        precision, recall, _ = precision_recall_curve(y, ypred1)
        PR1=auc(recall, precision)
        ROC1=roc_auc_score(y,ypred1)
        fpr.append(fpr1)
        tpr.append(tpr1)
        PR.append(PR1)
        ROC.append(ROC1)

      precision, recall, f1, _ = precision_recall_fscore_support(y, ypred, average='weighted')
      fscore.append(f1)
...
# seq_len=7
ModelTrain(dirname,SaveResults,'CNNSMILEGRU10times',seq_len=3,cnn=0,smile=1,diff=0,indx='test')

## 'CNNSMILEGRU10times' name of the model you wanna load 
