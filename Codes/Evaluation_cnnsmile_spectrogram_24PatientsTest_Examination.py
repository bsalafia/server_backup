...
def ModelTrain(dirname,dirname2,modeldir,SaveResults,SaveResultsTruePred,modelname,seq_len,cnn,smile,diff,twodcnn,spectrogram,indx,gru,epoch,l2_size,drop_size,batchSize):
  ...
  for th in [0.5]:
    ...
    for i in range(2):
      ...
      for k in range(len(x)):
        ...
        X_train=Xtrain(cnn,smile,diff,twodcnn,spectrogram,gru,X,Y,MI_all,MI_diff_all,spec1)
        ...
        print("Be careful################11111111111",len(x))
        time.sleep(5)
  ...
  for j in range(3):
      ...
      plt.suptitle(modelname+'_EDFNo_'+str(j+1)+'_epoch_'+str(epoch)+'_'+indx+'_batchsize_'+str(batchSize))

      plt.savefig(SaveResultsTruePred+'/'+modelname+'_EDFNo_'+str(j+1)+'_epoch_'+str(epoch)+'_'+indx+'_batchsize_'+str(batchSize)+'.pdf', format='pdf', bbox_inches = 'tight')
      plt.clf()
...
ModelTrain(dirname,dirname2,modeldir,SaveResults,SaveResultsTruePred,'CNNSMILE_24Testfiles',seq_len=1,cnn=0,smile=1,diff=0,twodcnn=0,spectrogram=0,indx='test',gru=0,epoch=300,l2_size=k,drop_size=i,batchSize=j)
