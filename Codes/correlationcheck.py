...
savedir='/home/baharsalafian/PearsonMeanCheckSymetry/'
...
for file_idx in [0,1,2,3,4]:
  ...
  _,_,X_train,Y_train=Label(dirname,EDFFiles[file_idx],WindowSize1,WindowSize2)
  ...
  for l in labels:
   ...
   for j in range(X_train.shape[2]-1):
    ...
    for k in range(j+1, X_train.shape[2]):
      ...
      estimated_MI[l,k,j] = np.mean(np.array(estimates))
      # time.sleep(0.5)
      print("-----File_idx {}, Label {}, Chan1 {},  Chan2 {}, Estimated MI {:.4f}".format(file_idx, l, j, k, estimated_MI[l,k,j]))

  Name=EDFFiles[file_idx].split('.')
  savemat(os.path.join(savedir, Name[0]+'MI_RollWindow_PearsonMean_Reversed.mat'), {"estimated_MI": estimated_MI})
