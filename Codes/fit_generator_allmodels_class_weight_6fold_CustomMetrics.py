...
def validation_batches(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn):
...
def validation_generator(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn):

  while True:
    x_batch,y_batch=validation_batches(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn)
    yield x_batch,y_batch
##################################3
def Train_batches_class_weight(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn):
...
def Train_Generator_ClassWeight(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn):

  while True:
    x_batch,y_batch=Train_batches_class_weight(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn)
    yield x_batch,y_batch
###################################
...
SaveHisResults='/home/baharsalafian/CustomMetrics_results_class_weight_6fold'
ModelResults='/home/baharsalafian/CustomMetrics_model_class_weight_6fold'


modelname='CNN_SMILE_NewArch_6Fold_class_weight_new_arch'
initLR=0.01
cnn=0
smile=0
smile_new_arch=1
...
for trainindx, testindx in kfold.split(range(24)):
  ...
  cw = class_weight.compute_class_weight('balanced', classes=np.unique(y_train),y=np.ravel(y_train))
  print(cw)
  class_weights = {0:cw[0], 1: cw[1]}

  x_train=Make_X(X_train,mi,spec_train,cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,twodcnn)
  x_test=Make_X(X_test, mitest,spec_test,cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,twodcnn)

  filename='history'+modelname+'_fold'+str(fold_no)+'_epoch_'+str(num_epochs)+'_LR_'+str(initLR)+'_batchsize_'+str(batchsize)+'.csv'

  # history_logger=tf.keras.callbacks.CSVLogger(filename, separator=",", append=False)

  # history=model.fit(x_train, y_train, validation_data=(x_test,y_test),batch_size=batchsize,epochs=num_epochs,class_weight=class_weights,callbacks=[LearningRateScheduler(lr_exp_decay,verbose=2)])
  TrainGen=Train_Generator_ClassWeight(X_train, y_train, mi,spec_train,batch_size,cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,twodcnn)  # TestGen=validation_generator(X_test, y_test, mitest,spec_test,batch_size,cnn,smile,spectrogram,cnnsmilespec,twodcnn)
  ...
  history=model.fit_generator(TrainGen,steps_per_epoch=train_steps,epochs=num_epochs,
                      ...
                      validation_data=validation_generator(X_test, y_test, mitest,spec_test,batch_size,cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,twodcnn),
                      validation_steps=test_steps,class_weight=class_weights)

  model.save(ModelResults+'/'+modelname+'_fold'+str(fold_no)+'_initLR_'+str(initLR)+'_epoch_'+str(num_epochs)+'_batchsize_'+str(batch_size)+'.h5')
  np.save(os.path.join(SaveHisResults, 'history'+modelname+'_fold'+str(fold_no)+'_epoch_'+str(num_epochs)+'_LR_'+str(initLR)+'_batchsize_'+str(batchsize)+'.npy'), history.history)
