...
def define_model_CNNSMILEGRU(l2_size,drop_size):
...
def define_model_CNNSMILEDiffGRU(l2_size,drop_size):
...
def ModelTrain(dirname,SaveResults,SaveHisResults,modelname,seq_len,cnn,smile,diff,twodcnn,epoch,l2_size,drop_size,batchSize):
  ...
  for trainindx, testindx in kfold.split(range(24)):
    ...
    if smile==1:
      tf.keras.backend.clear_session()
      model=define_model_CNNSMILEGRU(l2_size,drop_size)
      X_train=[x,mi]
      X_test=[xtest,mitest]
    ...
    if diff==1:
      ...
      tf.keras.backend.clear_session()
      model=define_model_CNNSMILEDiffGRU(l2_size,drop_size)
...
ModelTrain(dirname,SaveResults,SaveHisResults,'CNNGRU10times_lr.00001',seq_len=3,cnn=1,smile=0,diff=0,twodcnn=0,epoch=200,l2_size=k,drop_size=i,batchSize=j)
