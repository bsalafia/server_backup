...
################################################################
  ...
  model.compile(optimizer=Adam(learning_rate=0.0001),loss='binary_crossentropy',metrics=["accuracy", tf.keras.metrics.AUC(curve="ROC",name='auc_roc'),tf.keras.metrics.AUC(curve="PR",name='auc_pr'),f1_score_met,fbeta,precision,recall])
  return model
...
def ModelTrain(dirname,dirname2,SaveResults,SaveHisResults,modelname,seq_len,cnn,smile,diff,twodcnn,spectrogram,cnnsmilespec,gru,epoch,epochLoad,l2_size,drop_size,batchSize):
  ...
  for i in range(2):
    ...
    #
    #   print(k)
    x_train, y_train, mi_train,_,spec_train1=ReadMatFiles(dirname,dirname2,trainindx,seq_len,diff)
    print("spec",spec_train1.shape)
    # time.sleep(5)
    #####################over_sampling############################
    y_train=y_train.astype('int')
    nsamples, nx, ny = x_train.shape

    x_dataset = x_train.reshape((nsamples,nx*ny))
    spec_dataset=spec_train1.reshape((nsamples,nx*ny))
    oversample = RandomOverSampler(sampling_strategy='minority')
    sm = SMOTE(random_state=42)
    # mi, y= oversample.fit_resample(mi_train , y_train)
    # spec_over, y= oversample.fit_resample(spec_dataset , y_train)
    mi, y= sm.fit_resample(mi_train , y_train)
    spec_over, y= sm.fit_resample(spec_dataset , y_train)

    # X_over, y= oversample.fit_resample(x_dataset , y_train)

    X_over, y= sm.fit_resample(x_dataset , y_train)

    sample=y.shape[0]
    y=y.reshape(sample,1)

    x=X_over.reshape(sample,nx,ny)
    spec_train=spec_over.reshape(sample,nx,ny)
    ########################################################
    # time.sleep(5)
...
########################################
    ...
    if gru==1:
      history=model.fit(X_train, y, validation_data=(X_test,ytest), batch_size=batchsize, epochs=epoch, callbacks=[LearningRateScheduler(lr_exp_decay, verbose=2)])
    else:
    ...
    f1.append(history.history['f1_score_met'])
    f1_val.append(history.history['val_f1_score_met'])
...
SaveHisResults='/home/baharsalafian/History_JBHI_LossTest_Metrics_Complete'

ModelTrain(dirname,dirname2,SaveResults,SaveHisResults,'CNNSpec10times_Decay.5_LR.001_24Testfiles_SMOTE_Oversample',seq_len=1,cnn=0,smile=0,diff=0,twodcnn=0,spectrogram=1,cnnsmilespec=0,gru=0,epoch=150,epochLoad=0,l2_size=k,drop_size=i,batchSize=j)
