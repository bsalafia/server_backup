...
info_seizure = loadmat(os.path.join(savedir, 'seizure_duration_info.mat'))
nb_files_per_patient = info_seizure['nb_files_per_patient']
plt.plot(nb_files_per_patient.T[:,0])
plt.title('Number of files per patients')
plt.show()

res_gen_focal = [i for i in res_gen if i in res_focal]
print(len(res_gen_focal))
print(focal_no.T[res_gen_focal,0])
focal_no = np.delete(focal_no.T[:,0], res_gen_focal)
 
print(len(focal_no))

general_no = np.delete(general_no.T[:,0], res_gen_focal)
 
print("only generalized",
len([j for j, val in enumerate(general_no>0) if val]))

print("only focal",
len([j for j, val in enumerate(focal_no>0) if val]))
...
plt.title('Min and max of seizure duration')
plt.legend()
plt.xlabel('Patient ID')
plt.ylabel('seizure duration')
plt.show()
print("seizure shape is ", max(seizure_max))
