...
def FineTuning(SaveResults,modelname,fold_no,x1,y1,mi1,mi_diff1,batchsize,epoch,Xtest,MItest,MItest_diff,cnn,smile,diff,twodcnn,Noepoch):
  ...
  ModelName=modelname+'_fold'+str(fold_no)+'_epoch_'+str(Noepoch)+'.h5'
...
def ReadMatFiles(dirname,SaveResults,indx,fold_no,modelname,batchsize,epoch ,cnn,smile,diff,twodcnn,seq_len,Noepoch):
  ...
  for k in range(len(ind)):
    ...
    ypred,model=FineTuning(SaveResults,modelname,fold_no,x1,y1,mi1,mi_diff1,batchsize,epoch,Xtest2,MItest2,MItest_diff2,cnn,smile,diff,twodcnn,Noepoch)
...
def ModelTrain(dirname,SaveResults,SaveResultsTruePred,modeldir,indx,modelname,batchsize,epoch,cnn,smile,diff,twodcnn,seq_len,gru,Noepoch):
  ...
  savenamef1='fscore_'+modelname+indx+'_'
  cfname='cfmat_'+modelname+indx+'_'
  ...
  for th in [0.3,0.4,0.5,0.6,0.7]:
    ...
    for trainindx, testindx in kfold.split(range(24)):
      ...
      _,_,_,_,Ytest_all,Ypred_all,model=ReadMatFiles(dirname,modeldir,ind,fold_no,modelname,batchsize,epoch ,cnn,smile,diff,twodcnn,seq_len,Noepoch)

      model.save(modeldir+'/'+modelname+'_FineTune_'+str(fold_no)+'_'+str(Noepoch)+'.h5')
  ...
  for k in range(4):
    ...
    for trainindx, testindx in kfold.split(range(24)):
      ...
      fold_no=fold_no+1
      x,y,mi,mi_diff,_,_,model=ReadMatFiles(dirname,modeldir,ind,fold_no,modelname,batchsize,epoch ,cnn,smile,diff,twodcnn,seq_len,Noepoch)
    ...
    plt.suptitle(modelname+'_EDFNo_'+str(k+1)+'_'+indx)
    plt.savefig(SaveResultsTruePred+'/'+modelname+'_FineTune_'+'EDFNo_'+str(k+1)+'_epoch_'+str(Noepoch)+'_'+indx+'.pdf', format='pdf', bbox_inches = 'tight')
    plt.clf()
...
modeldir='/home/baharsalafian/CNNSMILEGRU100Epoch'

SaveResults='/home/baharsalafian/Results100Epoch_test'

SaveResultsTruePred='/home/baharsalafian/TruePredPlots_test'

ModelTrain(dirname,SaveResults,SaveResultsTruePred,modeldir,'_test_finetune','CNNGRU10times',batchsize=256,epoch=3,cnn=1,smile=0,diff=0,twodcnn=0,seq_len=3,gru=1,Noepoch=100)
