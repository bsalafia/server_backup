...
def define_CNN(initLR,drop_size):
  l2_size=None
  vectorsize=18
  input_shape=(1024,18)
  denseSize=8
...
def CNN_Spectrogram(initLR,filtNo1,filtNo2,filtsize,input_shape,activation,PoolSize,denseSize,l2_size,drop_size,stddev):
...
def CNN(initLR,filtNo1,filtNo2,filtsize,input_shape,activation,PoolSize,denseSize,l2_size,drop_size,stddev):
...
def ReadMatFiles(dirname,dirname2,indx, seq_len=3,diff=None):
...
def Make_X(x,mi,spectrogram,cnn,smile,spect,cnnsmilespec,smile_new_arch,eeg_smilemat,cnngru,cnnsmilegru,twodcnn):
...
def make_batches(x_one,x_zero,y_one,y_zero,mi_one,mi_zero,spectrogram_one,spectrogram_zero,mini_batch,cnn,smile,spect,cnnsmilespec,smile_new_arch,twodcnn):
  ...
  X_main=Make_X( x_batch,mi_batch,spectrogram_batch,cnn,smile,spect,cnnsmilespec,smile_new_arch,eeg_smilemat,cnngru,cnnsmilegru,twodcnn)
...
def validation_batches(x,y,mi,spectrogram,batch_size,cnn,smile,spect,cnnsmilespec,twodcnn):
  ...
  X_main=Make_X(x_one_batch,mi_one_batch,spectrogram_one_batch,cnn,smile,spect,cnnsmilespec,smile_new_arch,eeg_smilemat,cnngru,cnnsmilegru,twodcnn)
...
def Select_Model(cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,eeg_smilemat,cnngru,cnnsmilegru,twodcnn,diff,l2size,dropsize,stddev,initLR):

  if cnn==1:
    tf.keras.backend.clear_session()
    model=define_CNN(initLR,drop_size=dropsize)
  ...
  if spectrogram==1:
    tf.keras.backend.clear_session()
    model=CNN_Spectrogram(initLR,filtNo1=120,filtNo2=160,filtsize=5,input_shape=(1024,18),activation='relu',PoolSize=3,denseSize=8,l2_size=l2size,drop_size=dropsize,stddev=stddev)
...
SaveHisResults='/home/baharsalafian/LastExperiment_results_6fold'
ModelResults='/home/baharsalafian/LastExperiment_model_6fold'


modelname='CNNGRU_AddDrop_6Fold'
initLR=0.0001
cnn=0
cnngru=1
cnnsmilegru=0
...
for trainindx, testindx in kfold.split(range(24)):
  ...
  tf.keras.backend.clear_session()
  model=Select_Model(cnn,smile,spectrogram,cnnsmilespec,smile_new_arch,eeg_smilemat,cnngru,cnnsmilegru,twodcnn,diff,l2size,dropsize,stddev,initLR)
  ...
  wandb.init(project=modelname+"_l2size_"+str(l2size)+"_dropsize_"+str(dropsize)+"_Noise_"+str(stddev)+'_epoch_'+str(num_epochs)+"LR"+str(initLR), entity="bsalafian")
  #
  ...
  wandb.finish()
  model.save(ModelResults+'/'+modelname+'_fold'+str(fold_no)+'_epoch_'+str(num_epochs)+'_batchsize_'+str(batch_size)+'LR'+str(initLR)+'.h5')
