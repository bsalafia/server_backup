...
def Xtrain(cnn,smile,diff,smile_new_arch,twodcnn,spectrogram,gru,x,y,mi,spec):
...
def RemoveZero(dirprobs,nameProbPrefix,indx,fold_no):



  modelname=nameProbPrefix+'_'+indx+'_'+str(fold_no)

  mat=loadmat(os.path.join(dirprobs, modelname+'.mat'))
  length=mat['DataLength']
  pred=mat['prediction2']
  test=[]

  for i in range(len(length[0])):
      diff1=len(pred)-length[0,i]
      b1= [(p+length[0,i] ) for p in range(0, diff1)]
      test.append(np.delete(pred[:,i], b1))

  print("Len of Pred", len(test))
...
def ModelTrain(dirname,dirname2,dirprobs,modeldir,SaveResults,SaveResultsTruePred,nameProbPrefix,modelname,LR,seq_len,cnn,smile,diff,smile_new_arch,twodcnn,spectrogram,indx,gru,epoch,l2_size,drop_size,batchSize):
  ...
  for th in [0.7]:
    ...
    for testindx in range(24):
      ...
      # X_train=Xtrain(cnn,smile,diff,smile_new_arch,twodcnn,spectrogram,gru,Xtest,Ytest,MItest,Spectest)
      #
      # ModelName1=modelname+'_fold'+str(fold_no)+'_LR_'+str(LR)+'_epoch_'+str(epoch)+'_batchsize_'+str(batchSize)+'.h5'
      #
      ...
      ypred=RemoveZero(dirprobs,nameProbPrefix,indx,fold_no)
  ...
  #     Precision1, Recall1, f1, _ = precision_recall_fscore_support(Ytest, ypred_th, average='binary')
  ...
  #
  #
  #     plt.suptitle(modelname+'_PatientNo_'+str(j+1)+'_epoch_'+str(epoch)+'_'+indx+'_batchsize_'+str(batchSize))
  #
  #     plt.savefig(SaveResultsTruePred+'/'+modelname+'_PatientNo_'+str(j+1)+'_epoch_'+str(epoch)+'_'+indx+'_batchsize_'+str(batchSize)+'.pdf', format='pdf', bbox_inches = 'tight')
  #     plt.clf()
...
modeldir='/home/baharsalafian/LastExperiment_model_per_pataint'

SaveResults='/home/baharsalafian/Results_Allcase_JBHI_Lastexperiments'
SaveResultsTruePred='/home/baharsalafian/TruePredPlots_24Models_JBHI_CustomMetrics'

dirprobs='/home/baharsalafian/JBHI_FG_PerPatient_Results'

i=0
k=None
j=256

ModelTrain(dirname,dirname2,dirprobs,modeldir,SaveResults,SaveResultsTruePred,'JBHI_per_patients_train_CNN','CNNFG_per_pataint',LR=0.0001,seq_len=1,cnn=1,smile=0,diff=0,smile_new_arch=0,twodcnn=0,spectrogram=0,indx='test',gru=0,epoch=100,l2_size=k,drop_size=i,batchSize=j)
