...
#
  ...
  model.fit(mi_train,Y_train,validation_split=0.2,batch_size=batchsize , epochs=epoch,verbose = 2)

  X_train = None
  Y_train = None
  gc.collect()

  loss, acc = model.evaluate(mi_test,Y_test, verbose=2)
  score.append(acc)
  loss1.append(loss)
  model.save('/home/baharsalafian/CNNSMILE6FoldRes/CNN6foldsmileonly3times'+ str(fold_no)+'.h5')
  fold_no=fold_no+1
...
np.save(os.path.join(SaveResults,'accuracyCNN6foldsmileonly3times'),score)
np.save(os.path.join(SaveResults,'lossCNN6foldsmileonly3times'),loss1)
