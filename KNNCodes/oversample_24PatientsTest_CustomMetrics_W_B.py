...
modelname = 'TCN_CHB_MIT_On_Epoch_pkl'
initLR = 0.001
num_epochs = 10
batch_size = 256
...
# df_check = pd.read_pickle(os.path.join(os.path.join(dir_csv,'data_files_chb/test'), 'chb01_21_1.pkl'))

# print((df_check.iloc[0,1]))
...
TrainGen = validation_generator(x_train,y_train,batch_size)
...
model.fit_generator(TrainGen,steps_per_epoch=train_steps,epochs=num_epochs, callbacks=[WandbCallback()],
                    validation_data=validation_generator(x_test,y_test,batch_size), verbose = 2,validation_steps=test_steps)
