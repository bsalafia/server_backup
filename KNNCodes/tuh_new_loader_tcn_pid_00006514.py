...
def AUCPR(y_true, y_pred):
  ...
  return PR1
################################################3
###### you can uncomment this if you wanna use the custom one 
...
def creat_sub_seq_models(x,y,n_chan,one_sec_samp,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru):
  ...
  if tcn == 1:
    ...
    print("x shape for tcn is ",x_new.shape)
...
def create_csv_files_gru(dirname,all_img_dir,category_name,files,desired_intv,n_chan,one_sec_samp,len_seq,len_seq_gru,tcn,cnn,two_dcnn,gru):
  ...
  for i in range(len(files)):
    ...
    for j in range(x_new.shape[0]):
      ...
      print("this is test", y_save.shape)
      df = df.append({'FileName':files[i]+'_'+str(j), 'Label': y_save},ignore_index=True)
  df.to_pickle(os.path.join(os.path.join(dir_csv,f'data_files_tuh_selected_tcn/{category_name}'), f'{category_name}.pkl'))
  print("this is whole length", c)
...
class My_Custom_Generator(keras.utils.Sequence) :
  ...
  def __getitem__(self, idx) :
    ...
    x = np.array([loadmat('/home/baharsalafian/tuh_all_images_selected_tcn/' + str(file_name)+'.mat')['data'] for file_name in batch_x])
...
class My_Custom_Generator_train_gru(keras.utils.Sequence) :
  ...
  def __getitem__(self, idx) :
    ...
    x_one_total = np.array([loadmat('/home/baharsalafian/tuh_all_images_selected_tcn/' + str(file_name)+'.mat')['data'] for file_name in batch_x_one])
    x_zero_total = np.array([loadmat('/home/baharsalafian/tuh_all_images_selected_tcn/' + str(file_name)+'.mat')['data'] for file_name in batch_x_zero])
...
modelname = 'TCN_tuh_pid_00006514_no_oversample'
initLR = 0.0001
num_epochs = 150
batch_size = 256
...
model = TCN_Model(len_seq, one_sec_samp, num_channel, num_filters, initLR)
[print(i.shape, i.dtype) for i in model.inputs]
[print(o.shape, o.dtype) for o in model.outputs]
[print(l.name, l.input_shape, l.dtype) for l in model.layers]


df_valid = pd.read_pickle(os.path.join(dir_select, 'valid_pid_00006514.pkl'))


df_train = pd.read_pickle(os.path.join(dir_select, 'train_pid_00006514.pkl'))

train_files = df_train.file.tolist()
valid_files = df_valid.file.tolist()
...
# create_csv_files_gru(dirname,all_img_dir,'valid',valid_files,desired_intv,num_channel,one_sec_samp,len_seq,len_seq_gru,tcn,cnn,two_dcnn,gru)
# train_label = df.iloc[:,1]
# print("total label shape",train_label[1000])
# create_csv_files_gru(dirname,all_img_dir,'train',train_files,desired_intv,num_channel,one_sec_samp,len_seq,len_seq_gru,tcn,cnn,two_dcnn,gru)

start_time = time.time()

df_train = pd.read_pickle(os.path.join(os.path.join(dir_csv,'data_files_tuh_selected_tcn/train'), 'train.pkl'))
...
print("this is train label shape", train_label.shape)

df_test = pd.read_pickle(os.path.join(os.path.join(dir_csv,'data_files_tuh_selected_tcn/valid'), 'valid.pkl'))
# print(df_test.index.tolist())
test_file_name = df_test.iloc[:,0] 
test_label = df_test.iloc[:,1]


train_gen = My_Custom_Generator(train_file_name, train_label, batch_size,shuffle=True)
validation_gen = My_Custom_Generator(test_file_name, test_label, batch_size,shuffle=True)

# ind_zero = [i for i,x in enumerate(train_label) if (x==0).all()]
...
wandb.init(project = modelname+'_batch_size_'+str(batch_size)+'_num_epochs_'
                                +str(num_epochs)+'_LR_'+str(initLR)+'_num_filter_'+str(num_filters), entity="bsalafian")
...
model.save(ModelResults+'/'+modelname+'_epoch_'+str(num_epochs)+'_batchsize_'+str(batch_size)+'_LR_'+str(initLR)+'_num_filter_'+str(num_filters)+'.h5')
