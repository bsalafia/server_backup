...
def data_generator_test(data_main,batch_size,n_chan,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru):  
...
def data_generator_train(data_main,batch_size,n_chan,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru):  
  ...
  print(data2.shape)
  label=np.concatenate(label,axis=0)
  x_zero,y_zero,x_one,y_one = split(data2,label)
...
modelname = 'TCN_3times_validation'
initLR = 0.001
num_epochs = 50
batch_size = 128
num_filters = 16
...
gen_train = data_generator_train(names_train,batch_size,num_channel,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru)
gen_test = data_generator_test(names_test,batch_size,num_channel,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru)

model1 = TCN_Model(len_seq, one_sec_samp, num_channel, num_filters, LR)
model2=define_CNN(drop_size,LR)
...
wandb.init(project=modelname+'_len_seq_'+str(len_seq)+'_num_filters_'+str(num_filters)+'_batch_size_'+str(batch_size)+'_num_epochs_'
                                +str(num_epochs)+'_Nb_patients_'+str(focal_No+gen_No), entity="bsalafian")

model1.fit_generator(gen_train,epochs=num_epochs,steps_per_epoch=train_steps,
                     verbose=2,
                     callbacks=[WandbCallback(),LearningRateScheduler(lr_exp_decay)],
                     validation_data=gen_test,validation_steps=test_steps)

model1.save(ModelResults+'/'+modelname+'_len_seq_'+str(len_seq)+'_num_filters_'+str(num_filters)+'_batch_size_'+str(batch_size)+'_num_epochs_'
