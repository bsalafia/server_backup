...
def ModelTrain(dirname,save_grad_cam,dirname2,modeldir,SaveResults,SaveResultsTruePred,modelname,seq_len,cnn,smile,diff,twodcnn,spectrogram,cnnsmilespect,indx,gru,epoch,l2_size,drop_size,batchSize,noise):
  ...
  for th in [0.7]:
    ...
    for i in range(2):
      ...
      np.savez(f'{save_grad_cam}/data_{fold_no}.npz', X=x, Y=y, MI=mi, Spec=spec)
      # print(x[0].shape)
      # print(y[0].shape)
      # print(mi[0].shape)
      # print(spec[0].shape)
...
#           np.savez(os.path.join(SaveResults,'y_true_pred_test.npz'), y_true=Y, y_pred_th=ypred_th)
...
save_grad_cam = '/home/baharsalafian/data_grad_cam_all_case'
i=0
k=None
j=256
# n=0
ModelTrain(dirname,save_grad_cam,dirname2,modeldir,SaveResults,SaveResultsTruePred,'2DCNN_24Testfiles_init_0.0001',seq_len=1,cnn=0,smile=0,diff=0,twodcnn=1,spectrogram=0,cnnsmilespect=0,indx='test',gru=0,epoch=200,l2_size=k,drop_size=i,batchSize=j,noise=0)
