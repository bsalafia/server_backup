...
class DataGenerator(keras.utils.Sequence):
  ...
  def __data_generation(self,list_IDs_temp):
    ...
    for i,_ in enumerate(list_IDs_temp):
      ...
      data_path = self.data.iloc[i,0]
      y = self.data.iloc[i,1]
      # ind = self.data.iloc[i,2]
      # data_path = ast.literal_eval(data_path)
      # data_path = [n.strip() for n in data_path]
      # x = self.__segment_data(data_path[0])
      x = data_path
      # print(x.shape)
      # print("this is x shape", x.sh)
      X_data.append(x)
      y_data.append(y)
...
def validation_generator(dir_file_name,batch_size):
  x = []
  y = []
  c = 0
  while True:
    dir_file_name_shuffle = shuffle(dir_file_name)
    dir_file_name_select = dir_file_name_shuffle[0:10]

    for i in dir_file_name_select:
      df = pd.read_pickle(i)
      # print((np.array(df.iloc[:,0].values.tolist())).shape)
      x.append(np.array(df.iloc[:,0].values.tolist()))
      y.append(np.array(df.iloc[:,1].values.tolist()))
...
def dir_file_name_generate(dirname):

  os.chdir(dirname)
  file_names = glob.glob('*.pkl')
  dir_file_names = [os.path.join(dirname,i) for i in file_names]
  
  return dir_file_names

modelname = 'TCN_CHB_MIT_On_Epoch_pkl_random_file'
initLR = 0.001
num_epochs = 10
batch_size = 256
...
# dir_file_test = shuffle(dir_file_test)
# print(dir_file_name_generate(os.path.join(os.path.join(dir_csv,'data_files_chb/test/test_list_v2'))))




# df_train = pd.read_pickle(os.path.join(os.path.join(dir_csv,'data_files_chb/train/train_list_v2'), 'chb06_04_1.pkl'))
# x_test = np.array(df_train.iloc[:,1].values.tolist())
...
# x_test = np.array(df_test.iloc[:,0].values.tolist())
# y_test = np.array(df_test.iloc[:,1].values.tolist())

# x_train = np.array(df_train.iloc[:,0].values.tolist())
...
train_dir = os.path.join(dir_csv,'data_files_chb/train/train_list_v2')
test_dir = os.path.join(dir_csv,'data_files_chb/test/test_list_v2')

dir_file_train = dir_file_name_generate(train_dir)
dir_file_test = dir_file_name_generate(test_dir)

TrainGen = validation_generator(dir_file_train,batch_size)
...
model.fit_generator(TrainGen,steps_per_epoch=train_steps,epochs=num_epochs, callbacks=[WandbCallback()],
                    validation_data=validation_generator(dir_file_test,batch_size), verbose = 2, validation_steps=test_steps)
