...
def ReadMatFiles(dirname,dir_csv,ind,category_name):
  ...
  for k in range(len(ind)):
    ...
    for j in range(x.shape[0]):
      df = df.append({'FileName':x[j,:,:] , 'Label': real_y[j,:][0],'index': np.int64(j)},ignore_index=True)
    df.to_pickle(os.path.join(os.path.join(dir_csv,f'data_files_chb_old/{category_name}'), f'{csv_name[0]}.pkl'))
    c = c + len(df) - 7
...
def load_samples_modified(dir_csv,data_cat,data_list,temporal_length,temporal_stride,shuffle=True):
  ...
  data_path = os.path.join(dir_csv,f'data_files_chb_old/{data_cat}')
  ...
  df.to_pickle(os.path.join(os.path.join(dir_csv,f'data_files_chb_old/test/test_list'), 'data_list.pkl'))


def load_samples_modified_chb_old(dir_csv,data_cat,data_list,temporal_length,temporal_stride,shuffle=True):

  data_path = os.path.join(dir_csv,f'data_files_chb_old/{data_cat}')
  data_csv_list = os.listdir(data_path)
  os.chdir(data_path)
  data_csv_list = glob.glob('*.pkl')
  ...
  for f in data_files:
    ...
    for j in range(len(f)):
      df = df.append({'FileName': x[j], 'Label': y[j]},ignore_index=True)
  
  df.to_pickle(os.path.join(os.path.join(dir_csv,f'data_files_chb_old/train/train_list'), 'data_list.pkl'))
  ...
  for i in range(len(data_files)):
    ...
    while iterator:
        try:
            ...
            iterator = False
            print ('end of data generator')
    df.to_pickle(os.path.join(os.path.join(dir_csv,f'data_files_chb_old/train/train_list_v2'), data_files[i]))
...
class DataGenerator(keras.utils.Sequence):
  ...
  def __data_generation(self,list_IDs_temp):
    ...
    # print(type(np.array(y.values.tolist())))
...
modelname = 'CNN_CHB_MIT_On_Epoch_pkl_test2'
...
if not os.path.exists(os.path.join(dir_csv,'data_files_chb_old/test/test_list')):
    os.mkdir(os.path.join(dir_csv,'data_files_chb_old/test/test_list'))

if not os.path.exists(os.path.join(dir_csv,'data_files_chb_old/train/train_list')):
    os.mkdir(os.path.join(dir_csv,'data_files_chb_old/train/train_list'))

if not os.path.exists(os.path.join(dir_csv,'data_files_chb_old/test/test_list_v2')):
    os.mkdir(os.path.join(dir_csv,'data_files_chb_old/test/test_list_v2'))


if not os.path.exists(os.path.join(dir_csv,'data_files_chb_old/train/train_list_v2')):
    os.mkdir(os.path.join(dir_csv,'data_files_chb_old/train/train_list_v2'))
...
# load_samples_modified_chb_old(dir_csv,data_cat='train',data_list='train_list',shuffle=True,temporal_length=8,temporal_stride=1)
df_train = pd.read_pickle(os.path.join(os.path.join(dir_csv,'data_files_chb_old/train/train_list'), 'data_list.pkl'))

# y = df_train.iloc[:,1]
# y = np.array(y.values.tolist())
# y = np.expand_dims(y, axis=1)
# print(len(df_train))
df_test = pd.read_pickle(os.path.join(os.path.join(dir_csv,'data_files_chb_old/test/test_list'), 'data_list.pkl'))
...
model.fit_generator(train_gen,epochs=num_epochs, callbacks=[WandbCallback(),LearningRateScheduler(lr_exp_decay)],
