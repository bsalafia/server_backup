...
def Readpklfiles(dirname,index,n_chan,one_sec_samp,len_seq,len_seq_gru,tcn,cnn,two_dcnn,gru):
    ...
    for i in range(len(index_v2)):
      ...
      sig_segment = []
      for j in range(0, data_len, fs):
        sig_segment.append(sig[:,j:j+fs])
...
data_test, label_test = Readpklfiles(dirname,test,num_channel,one_sec_samp,len_seq,len_seq_gru,tcn,cnn,two_dcnn,gru)

print("the size of test data is ", sys.getsizeof(data_test))

data_train, label_train = Readpklfiles(dirname,train,num_channel,one_sec_samp,len_seq,len_seq_gru,tcn,cnn,two_dcnn,gru)
print("the size of train data is ",sys.getsizeof(data_train))
...
cw = class_weight.compute_class_weight(class_weight='balanced', classes=np.unique(label_train), y=np.ravel(label_train))
class_weights = {0:cw[0], 1: cw[1]}
wandb.init(project=modelname+"LR"+str(initLR)+'_epoch_'+str(num_epochs)+'_nb_filter_'+str(num_filters)+'_'+'len_seq'+str(len_seq), entity="bsalafian")


history = model.fit(x=data_train, y=label_train, validation_data=(data_test,label_test), batch_size=batchsize, epochs=num_epochs, callbacks=[WandbCallback()], verbose = 2)
