...
def data_generator_train(dirname,ind,num_channel,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru,batch_size):  

  train_steps = int(31395//batch_size)
  cnd = 0
  while True:

    cnd = cnd + 1
    print("****************this is for while counter ", cnd)

    
      
      # print("this is for test as counter ", i)
    EDF = []
    EDFFiles = []
    Name = []
    EDF = PatientsEDFFile(dirname)
    ...
    num_samples = 10
    EDF_selected = [EDF[i] for i in ind]
    data_shuffle = shuffle(EDF_selected)
    data_samples = data_shuffle[0:num_samples]
    #print(data_samples)

    for k in range(len(data_samples)):

      # print(data_samples[k])
      x, y = segment_data(dirname,data_samples[k])
      X.append(x)
      Y.append(y)
    ...
    x_zero, y_zero, x_one, y_one = split(x_new, y_new)
...
def data_generator_test(dirname,ind,num_channel,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru,batch_size):  
  test_steps=int(31395//batch_size)
  ...
  while True:
    ...
    total_sample = x_new.shape[0]
    selected_idx = np.random.choice(total_sample,batch_size)
    data_batch = x_new[selected_idx]
    label_batch = y_new[selected_idx]
...
modelname = 'TCN_CHB_MIT_data_gen_test'

i = 0
testindx,trainindx=SelectIndx(i,indx,loaddir)

TrainGen = data_generator_train(loaddir,testindx,num_channel,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru,batch_size)
ValidGen = data_generator_test(loaddir,testindx,num_channel,tcn,cnn,two_dcnn,gru,len_seq,len_seq_gru,batch_size)
# train_steps=int(160797//batch_size)
train_steps=int(31395//batch_size)
test_steps=int(31395//batch_size)
